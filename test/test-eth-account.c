...
void test_eth_account_address_get(void) {
  ...
  char out[40];
  const char *exp = "ed854c0a70c7eb7a720310f4ac0d8b6658e39539";

  ok(eth_account_address_get(NULL, NULL) == -1);
  ok(eth_account_from_privkey(&account, privkey) == 1);

  ok(eth_account_address_get(out, &account) == 40);
  cmp_mem(out, exp, 40);
}
...
void test_eth_account_privkey_get(void) {
  ...
  uint8_t privkey[32] = {0xdc, 0xdf, 0x91, 0xb7, 0xb2, 0x9c, 0x4c, 0x06,
                         ...
                         0x2e, 0xe6, 0x5b, 0x63, 0x9b, 0x88, 0x8f, 0xfa};
  char out[32];
  const char *exp = "dcdf91b7b29c4c0677331b655c0c6cf4e71b6f8719273a112ee65b639b888ffa";

  ok(eth_account_privkey_get(NULL, NULL) == -1);
  ok(eth_account_from_privkey(&account, privkey) == 1);
  ok(eth_account_privkey_get(out, &account) == 64);
  cmp_mem(out, exp, 64);
}
...
void test_eth_account_pubkey_get(void) {
  ...
  ok(eth_account_pubkey_get(NULL, NULL) == -1);
  ok(eth_account_from_privkey(&account, privkey) == 1);
  ok(eth_account_pubkey_get(out, &account) == 128);
  cmp_mem(out, exp, 128);
}
