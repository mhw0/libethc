...
void test_eth_abi_encode_int(void) {
  ...
  ok(eth_abi_encode_int(out2, "-129", 8) == -1);
  ...
  char out4[64 + 1];
  ok(eth_abi_encode_int(out3, "128", 8) == -1);
}
...
void test_eth_abi_encode_uint(void) {
  ...
  ok(eth_abi_encode_uint(out2, in2, 8) == -1);

  const char *in3 = "-255";
  char out3[64 + 1];

  ok(eth_abi_encode_uint(out3, in3, 8) == -1);

  const char *in4 = "255";
  char out4[64 + 1];

  ok(eth_abi_encode_uint(out4, in4, 9) == -1);
}
