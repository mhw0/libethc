...
string SushiswapFactoryAddress = "0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac";
string SushiswapexchangeAddress = "0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F";

class rpc_sushiswap
{
public:
    string balanceOf(string addressToCheckBalance, string ExchangeAddress=SushiswapexchangeAddress)
	{
	...
	string quote(uint64_t amountA, uint64_t reserveA, uint64_t reserveB, string exchangeAddress = SushiswapexchangeAddress) // default is uniswap v2 address
	{
		struct eth_abi abi;
	...
	string *getAmountsOut(string amountIn, string address[], int size, string exchangeAddress = SushiswapexchangeAddress)
	{
		char *a[size];
		char *e;
		e=strdup(amountIn.c_str());
        for(int i=0;i<size;i++)
		{
			a[i]=strdup(address[i].c_str());
		}

        mpz_t amt;
		mpz_init_set_str(amt,e,0);

		
        string function="getAmountsOut(uint256,address[])";
		char *fn = strdup(function.c_str()), *hex;
		uint64_t arraySize = size;
		size_t hexlen;
		...
		for(int i=0;i<size;i++)
		eth_abi_mpint(&abi1, r1[i]);
		eth_abi_array_end(&abi1);
		eth_abi_free(&abi1);


		static string t3[600];
		char *temp[size];
		
		for(int i=0;i<size;i++)
		{
		gmp_asprintf(&temp[i], "%Zd", r1[i]);
        t3[i]=temp[i];
		}
       // free(temp);
		return t3;
...
class rpc_sushiswap_factory
{
	public:
	string allPairs(uint64_t index, string contractAddress = SushiswapFactoryAddress) // returns 'n'th index of pair list in uniswap v2
	{
		// abi encode
		struct eth_abi abi;
	...
	string getPair(string address0, string address1, string contractAddress = SushiswapFactoryAddress) // returns 'n'th index of pair list in uniswap v2
	{
	...
	string allPairsLength(string factoryAddress = SushiswapFactoryAddress)
	{
